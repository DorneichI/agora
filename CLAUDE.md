...
This job is skipped for Dependabot PRs (`github.actor == 'dependabot[bot]'`) — their PR body is
auto-generated by GitHub and never matches our template's headers, and there's no way to make
Dependabot itself write to a custom template. `ci-status` treats a skipped job the same as a
passing one, so this doesn't weaken the gate: the actual code checks (`backend`/`web`) still run
and block the merge on a Dependabot PR exactly as they would on any other.
