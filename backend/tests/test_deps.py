...
from app.deps import require_admin, require_league_admin, require_league_owner
...
async def test_require_league_admin_rejects_non_member(db_session):
    creator = User(clerk_id="user_rla_1", email="rla1@example.com", display_name="Creator")
    other = User(clerk_id="user_rla_2", email="rla2@example.com", display_name="Other")
...
async def test_require_league_admin_rejects_plain_member(db_session):
    creator = User(clerk_id="user_rla_3", email="rla3@example.com", display_name="Creator")
    member = User(clerk_id="user_rla_4", email="rla4@example.com", display_name="Member")
...
async def test_require_league_admin_allows_admin(db_session):
    creator = User(clerk_id="user_rla_5", email="rla5@example.com", display_name="Creator")
...
async def test_require_league_admin_missing_league_returns_404(db_session):
    user = User(clerk_id="user_rla_6", email="rla6@example.com", display_name="Nobody")
    db_session.add(user)
    await db_session.commit()
...
async def test_require_league_owner_rejects_non_owner_admin(db_session):
    owner = User(clerk_id="user_rlo_1", email="rlo1@example.com", display_name="Owner")
    admin = User(clerk_id="user_rlo_2", email="rlo2@example.com", display_name="Admin")
...
async def test_require_league_owner_allows_owner(db_session):
    owner = User(clerk_id="user_rlo_3", email="rlo3@example.com", display_name="Owner")
